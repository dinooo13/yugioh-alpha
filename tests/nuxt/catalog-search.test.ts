...
import { getCatalogCardDetail, getCatalogFacets, searchCatalogCards } from '../../server/utils/catalog-search'
...
describe('catalog search utilities', () => {
  ...
  it('filters and paginates catalog cards', async () => {
    const result = await searchCatalogCards(db, parseCardListQuery({
      type: 'Spell Card',
      pageSize: '1',
      page: '2',
    }))
  ...
  it('searches card names case-insensitively', async () => {
    const result = await searchCatalogCards(db, parseCardListQuery({ q: 'blue' }))
  ...
  it('treats LIKE wildcards in search input as literal characters', async () => {
    const result = await searchCatalogCards(db, parseCardListQuery({ q: '50%_off' }))

    expect(result.items.map(card => card.name)).toEqual(['50%_off Dragon'])
  })

  it('filters through printings by set id', async () => {
    const result = await searchCatalogCards(db, parseCardListQuery({ setId: 'structure-deck-kaiba' }))
