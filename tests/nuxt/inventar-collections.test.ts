...
// Collection management on /inventar (#41): the URL (`?collectionId=`) is the
...
describe('inventory collection scope from the URL', () => {
  it('scopes header, both views and the presets to ?collectionId=', async () => {
    const component = await mountSuspended(InventarPage, { route: '/inventar?collectionId=col-1' })
    ...
    expect(quickEntry!.attributes('href')).toBe('/inventar/erfassen?collectionId=col-1')
  ...
  it('shows the cards without a collection for ?collectionId=__none__', async () => {
    const component = await mountSuspended(InventarPage, { route: '/inventar?collectionId=__none__' })
    ...
    const quickEntry = component.findAll('a').find(link => link.text().includes('Schnellerfassung'))
    expect(quickEntry!.attributes('href')).toBe('/inventar/erfassen')
  ...
  it('drops an unknown ?collectionId= once the collections are loaded', async () => {
    await mountSuspended(InventarPage, { route: '/inventar?collectionId=gone&view=uebersicht' })

    await vi.waitFor(() => {
      expect(currentQuery()).toEqual({ view: 'uebersicht' })
    })
  })
})
...
describe('inventory collection management', () => {
  ...
  it('deletes the active collection after a confirmation and falls back to all cards', async () => {
    const component = await mountSuspended(PageWithConfirmDialog, { route: '/inventar?collectionId=col-1' })
